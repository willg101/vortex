 ...
 * @return string
 */
function convertToCodebaseRelativePath (path, codebaseId, codebaseRoot) {
  var pathStripped = stripScheme(path).replace(/^\/+/, '/')
  codebaseRoot = stripScheme(codebaseRoot).replace(/^\/+/, '/')
 ...
 * @return string
 */
function convertFromCodebaseRelativePath (path, codebaseId, codebaseRoot) {
  codebaseId += CODEBASE_RELATIVE_PATH_ID_SEPARATOR
  if (!path.startsWith(codebaseId)) {
    return path
  }
  return codebaseRoot.replace(/\/*$/, '/') + path.replace(codebaseId, '')
}
